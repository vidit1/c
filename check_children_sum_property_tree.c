...
//children_sum function runs recursively for each node and check whether children sum property on it
int children_sum(node *node)
{
    //if the node is NULL or the node is a leaf node than return 1 which basicaly means it is true
    if(node==NULL||(node->right==NULL&&node->left==NULL))
        return 1;
    //intializing the variables which stores data of left and right node of parent node to 0 as if they are NULL the value will be 0
    int left=0,right=0;
    //if right of parent node is not null store data in right
    if(node->right!=NULL)
        right=node->right->data;
    //if left of parent node is not null store data in left
    if(node->left!=NULL)
        left=node->left->data;
    //check if sum of left and right is equal to parent node data and recusively calling the function for left and right node
    if((node->data==left+right)&&children_sum(node->left)&&children_sum(node->right))
        return 1;
    //if condition does not satifies than return 0
    else
        return 0;
}
int main()
{   //intializing different variables accordingly
    ...
    //printing the tree
    print_inorder(root);
    //calling children sum property to check whether the tree satisfoes the property or not
    //if the value of flag is 1 then condition is satisfied
    flag=children_sum(root);
    if(flag==1)
    {
        printf("\nChildren Sum property satisfies");
    }
    else
    {
